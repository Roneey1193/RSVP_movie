...
This are the corresponding null values present for respective column *\ 
...
SELECT genre,
	    count(distinct(id)) as no_of_movie 
...
-- Genre Drama as most number of movies 
...
-- 3289 movie id have single genre 
...
-- crime has highest avg_duration 
...
-- Thriller has Rank 3 with total movies of 1484 
...
-- This are top 10 movies based on avg_rating.
...
-- movie with median_rating 7 is highest in number.
...
-- Dream Warrior Pictures and National Theatre live  as highest  movie count that are produce 
...
from movie as m inner join ratings r 
on m.id=r.movie_id
inner join
genre as g
on m.id=g.movie_id
...
SELECT *,
       Rank()
         OVER(
           ORDER BY actress_avg_rating DESC) AS actor_rank
FROM   actor_summary; 
